...
const Invoice = ()=>{
    ...
    const saveInvoice = async ()=>{
        ...
        await SaveInvoice(invoiceOutput);
    }
    ...
    return (
        <div className={"voucherGrid"} style={{ display: "flex", flexDirection:"column",}}>
            ...
            <div className={"party-row"} style={{ display:"flex", alignItems:"center",
            ...
            <div className={"item-rows"} style={{ maxHeight:"500px", overflowY:"scroll"}}>
                {
                    items.map((item,i)=>
                        <ItemRow key={item.key} deleteRow={deleteRow} handleChange={handleItemChange} index={i} item={item}/>)
                }
            </div>
            <div>
                <Button onClick={()=>{
                ...
                <Button onClick={async ()=>{await saveInvoice();}}>Save</Button>
            </div>
...
const ItemRow: FC<ItemRowProps> = ({ index, item, handleChange, deleteRow })=>{
    ...
    return(
        ...
        >
            <Button type={"danger"} onClick={()=>{deleteRow(index)}}>
                delete
            </Button>
            <SkuSelect
                name={selected?.name}
            ...
            <input className={"covert-input"}
                   ...
                   key={rate}
                   defaultValue={rate} placeholder={"Rate b/f tax"}/>
            <input
                ...
                className={"covert-input"} placeholder={"Discount"}/>
            <input disabled  className={"covert-input"} value={finalRate} />
            <input disabled value={rateAfterTax} className={"covert-input"}/>
        </div>
    )
}
